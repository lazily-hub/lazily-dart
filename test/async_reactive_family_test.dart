...
/// [AsyncSourceMap] / [AsyncComputedMap] specializations (`#reactivemap`, async).
...
/// ([AsyncComputedMap.materializeAll]); lazy = mint-on-access ([AsyncComputedMap.touch]
/// + [AsyncComputedMap.drive]) — no mode flag.
void main() {
  group('AsyncComputedMap resolution axis', () {
    test('eager slot is present-but-pending, resolves after drive', () {
      // observe_pending_is_none, then eventual_transparency.
      final ctx = Context();
      final fam = AsyncComputedMap<int, int>(ctx)..materializeAll([1]);
    ...
    test('lazy slot is absent until touched, then pending until driven', () {
      final ctx = Context();
      final fam = AsyncComputedMap<int, int>(ctx);
  ...
  group('AsyncSourceMap cell entries', () {
    ...
    test('set on a cell overwrites and stays resolved', () {
      final ctx = Context();
      final fam = AsyncSourceMap<int, int>(ctx);
  ...
  group('AsyncComputedMap resolve monotonicity', () {
  ...
  group('AsyncComputedMap eventual transparency', () {
  ...
  group('AsyncComputedMap present-set', () {
